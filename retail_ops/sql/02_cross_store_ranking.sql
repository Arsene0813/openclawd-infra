...
SELECT
    ...
    search_visit_share,
    RANK() OVER (ORDER BY search_visit_share DESC)
        AS rank_search_visit_share,

    search_exposure_to_visit_rate,
    RANK() OVER (ORDER BY search_exposure_to_visit_rate DESC)
        AS rank_search_exposure_to_visit_rate,

    refund_revenue_ratio,
    RANK() OVER (ORDER BY refund_revenue_ratio ASC)
        AS rank_low_refund_revenue_ratio,

    refund_order_ratio,
    RANK() OVER (ORDER BY refund_order_ratio ASC)
        AS rank_low_refund_order_ratio,

    subsidy_to_revenue_ratio,
    RANK() OVER (ORDER BY subsidy_to_revenue_ratio ASC)
        AS rank_low_subsidy_to_revenue_ratio,

    revenue_per_visitor,
    RANK() OVER (ORDER BY revenue_per_visitor DESC)
        AS rank_revenue_per_visitor
FROM derived
